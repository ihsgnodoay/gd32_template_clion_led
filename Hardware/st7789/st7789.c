...
void ST7789_SPI_Init(){
  ...
  gpio_output_options_set(GPIOF, GPIO_OTYPE_PP, GPIO_OSPEED_25MHZ, GPIO_PIN_9);
	gpio_bit_set(GPIOF, GPIO_PIN_9);
	
	// PB0 SCL
	gpio_af_set(GPIOB, GPIO_AF_6, GPIO_PIN_0);
	gpio_mode_set(GPIOB, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_PIN_0);
  gpio_output_options_set(GPIOB, GPIO_OTYPE_PP, GPIO_OSPEED_25MHZ, GPIO_PIN_0);
	gpio_bit_set(GPIOB, GPIO_PIN_0);
	
	rcu_periph_clock_enable(RCU_SPI4);
	spi_parameter_struct spi_init_struct;
	spi_i2s_deinit(ST7789_SPI_PORT);
	...
	spi_init(ST7789_SPI_PORT, &spi_init_struct);

	spi_enable(ST7789_SPI_PORT);	
}

void ST7789_SPI_Write(uint8_t dat){		
		
		/* loop while spi tx data register in not emplty */
    while (RESET == spi_i2s_flag_get(ST7789_SPI_PORT, SPI_FLAG_TBE)) ;
		/* send byte through the SPI2 peripheral */
    spi_i2s_data_transmit(ST7789_SPI_PORT, dat);
    while (RESET == spi_i2s_flag_get(ST7789_SPI_PORT, SPI_FLAG_TBE)) ;
